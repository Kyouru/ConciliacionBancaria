...
Private Sub btAceptar_Click()
    If ListBox1.ListIndex <> -1 Then
        ...
        OpenDB
        strSQL = "SELECT PATH FROM CONFIG_PATH WHERE NOMBRE_PATH = 'XLSX_PATH'"
        ...
        'Log del Error
        logError cnn, Me.Name, "btAceptar_Click"
        
        If rs.RecordCount > 0 Then
            xlsxPath = rs.Fields("PATH")
        End If
        ...
        strDir = xlsxPath & Format(tempDate, "YYYY") & "\" & Format(tempDate, "MM") & "\" & Format(tempDate, "YYYY.MM.DD") & "\" & tmpXLSX & ".xlsx"
...
Private Sub btBuscar_Click()
        ...
        If Me.lbFecha.Caption <> "" Then
            If IsDate(Me.lbFecha.Caption) Then
                strSQL = strSQL & " AND FORMAT(FECHA, 'YYYY-MM-DD') = '" & Format(Me.lbFecha.Caption, "YYYY-MM-DD") & "'"
            End If
        End If
...
Private Sub btCalendario_Click()
    ...
    If Not IsNull(Hoja1.UltimaFecha) Then
        Me.lbFecha.Caption = Format(Hoja1.UltimaFecha, "DD/MM/YYYY")
    End If
...
Private Sub UserForm_Initialize()
    lbFecha.Caption = Format(Now(), "DD/MM/YYYY")
    btBuscar_Click
End Sub
